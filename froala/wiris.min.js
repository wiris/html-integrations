...
var _wrs_conf_path = 'http://localhost/plugin/3.50/froala_wiris/froala/js/plugins/froala_wiris/';
console.log(_wrs_conf_path);
...
(function ($) {
        ...
        function waitForCore() {
            if (window.wrs_initParse && typeof _wrs_conf_plugin_loaded != 'undefined') {
              if (this.$iframe != null) {
                ...
                var parsedContent = wrs_initParse(editor.$box.froalaEditor('html.get'));
                editor.$box.froalaEditor('html.set', parsedContent);

                editor.$box.on('froalaEditor.html.get', function(e, editorm, html) {
                    return wrs_endParse(html);
                });
  ...
  // Add an option for your plugin.
  $.FroalaEditor.DefineIcon('wirisEditor', {SRC: _wrs_conf_path +'/icons/formula.png', ALT: 'Image button', template: 'image'});
  $.FroalaEditor.RegisterCommand('wirisEditor', {
      ...
      callback: function () {
        wrs_int_openNewFormulaEditor(this.$iframe != null ? this.$iframe[0] : this.$box[0], 'en', this.$iframe != null ? true : false);
      }
    });


  $.FroalaEditor.DefineIcon('wirisChemistry', {SRC: _wrs_conf_path +'/icons/chem.png', ALT: 'Image button', template: 'image'});
...
function wrs_int_insertElementOnSelection() {
  var inst = $.FroalaEditor.INSTANCES[0];
  inst.$el.focus();
}
...
function wrs_int_getSelectedItem(target, isIframe) {
  if ($('#example').froalaEditor('image.get') !== null && $('#example').froalaEditor('image.get').hasOwnProperty('0')) {
    var selectedItem = new Object();
    selectedItem.node = $('#example').froalaEditor('image.get')[0];
    return selectedItem;
  } else {
